...
#include "cat_dog_detect_model_data.h"
...
void setup() {
  ...
  model = tflite::GetModel(g_cat_dog_detect_model_data);
...
void loop() {
  ...
  uint8_t cat_score = output->data.uint8[kCatIndex];
  uint8_t dog_score = output->data.uint8[kDogIndex];
  RespondToDetection(error_reporter, cat_score, dog_score);
